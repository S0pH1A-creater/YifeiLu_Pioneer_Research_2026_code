...
def _stock_metrics(paths, hist) -> dict:
    ...
    return {
        "rmse_st": float(np.sqrt(np.mean((p50 - hist_v) ** 2))),
...
def _stop_rmse(g: dict, ticker: str, cal: pd.DataFrame, contracts: pd.DataFrame) -> dict:
    ...
    return {
        "rmse_stop": float(np.sqrt(np.mean(err ** 2))),
...
def main() -> int:
    ...
    rpt.N_CONTRACTS = N_CONTRACTS

    rows: list[dict] = []
    t_all = time.time()
    print(
        f"2022-10-21 | lookback={WINDOW_LABEL} | rolling={list(ROLLING_MODES)} | "
        f"stock_paths={N_PATHS_STOCK} | stop_paths={N_PATHS_STOP} | n≤{N_CONTRACTS}",
        flush=True,
    )
