...
Writes:
  V2-Models_result/results/7d_1min/<window_id>/
...
SHORT = REPO / "Results_In_Short" / "7 days regimes" / WINDOW_ID
...
def run_stopping(nb_path: Path, model: str, g: dict, contracts_by: dict) -> dict[str, tuple]:
    ...
    for ticker in TICKERS:
        ...
        for mode in ROLLING_MODES:
            ...
            modes[mode] = packed
            print(
                f"    stop {ticker}/{mode}: RMSE={packed['rmse']:.4f} "
...
**Source:** `V2-Models_result/results/7d_1min/{WINDOW_ID}/parameters/{mode}/`  
...
**Window:** `{WINDOW_ID}`.  
**Source:** `V2-Models_result/results/7d_1min/{WINDOW_ID}/{{TICKER}}/`  
Same listed contracts / seeds / LSM harness for **SPY**, **AAPL**, and **MSFT** (n={N_CONTRACTS}, seed={SEED}, n_paths={N_PATHS}; DTE 7–60 from the short_interval panel).

**RMSE is model LSM vs market `option_price`**, same decision metric as the 2-year notebooks.
...
"""
        ...
        for ticker in TICKERS:
            ...
            for i, (model, regime, stem, modes) in enumerate(completed, start=1):
                m = modes[mode]
                table.append(
                    f"| {i} | {regime} | {model} | **{m['rmse']:.4f}** | {m['mae']:.4f} | {m['bias']:.4f} |"
                )
...
def build_all_tables(by_ticker: dict) -> dict[str, list[dict]]:
    ...
    for ticker in TICKERS:
        ...
        for model, regime, stem, modes in completed:
            ...
            lines.append(
                f"| {regime} | {model} | {modes['hourly']['rmse']:.4f} | "
                f"{modes['minutely']['rmse']:.4f} | `{best}` | [{stem}.md]({stem}.md) |"
            )
            rows.append(
                {
                    "Regime": regime,
...
def build_rmse_bars(parsed: dict[str, list[dict]]) -> None:
    ...
    def plot_one(ax, ticker: str) -> None:
        ...
        ax.set_ylabel("RMSE")
        ax.set_title(f"{ticker} — RMSE by model · {WINDOW_ID}")
...
Canonical numbers/figures: `V2-Models_result/results/7d_1min/{WINDOW_ID}/`.
