"""Shared listed-call filters for V3 parameter estimation.

Applied to every model that uses option quotes (Heston / Heston–Merton
Fourier calibration, and the LSM contract sample used for comparison).
Return-based estimators (GBM, Merton, GARCH, GARCH–Merton) do not see
these quotes for §4; they still use the same filtered set in §6.

Steps (in order):
  1. No-arbitrage: C ≥ max(0, S − K)
  2. Maturity: 7 ≤ DTE ≤ 60
  3a. Moneyness: |S/K − 1| ≤ 10%
  3b. Liquidity: valid bid/ask and relative spread ≤ 50% when those
