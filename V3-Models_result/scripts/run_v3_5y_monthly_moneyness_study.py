...
def _model_rows(rows: list[dict]) -> tuple[list[list[str]], list[str], int | None]:
    header = ["Model", "n", "RMSE%", "MAE", "Bias $", "Bias%", "Early-ex.", "Rank"]
    out = []
    rmses = []
    for rec in rows:
        ...
        out.append(
            [
                ...
                _fmt(rec["bias"], "bias") if rec["n"] else "—",
                _fmt(rec["bias_pct"], "bias_pct") if rec["n"] else "—",
                _fmt(rec["early"], "early") if rec["n"] else "—",
                "—",
            ]
        )
...
def _cover(pdf, payload: dict) -> None:
    ...
    y = section(
        ...
        [
            ...
            "MAE and Bias $ in option-premium dollars. Bias% = 100 × mean((C_model − C_mkt)/C_mkt). Early-exercise fraction = mean LSM share of paths that stop before expiry.",
        ],
        y,
    )
    y = section(
...
def _analysis1_tables(pdf, payload: dict) -> None:
    ...
    fig.text(0.06, 0.025, "Bias $ = mean(model − market).  Bias% = 100 × mean((model − market)/market).", fontsize=7.8, color="#555555")
