# Merton notebooks — P from returns, Q via Pan (2002)

V4 only. \(P\): existing 3σ jump estimator on lookback returns. **Jump-size premium** \(\mu_J^*\) from listed calls (Pan 2002). \(\lambda^*=\lambda\), \(\sigma^Q=\sigma^P\), \(\sigma_J^Q=\sigma_J^P\). Do not set \(P=Q\) for jumps when \(\lambda>0\).

§4 graphs \(P\). §5 Monte Carlo is \(P\). §6 LSM uses \(Q\) (`simulate_merton_q`): drift \(r_f-\lambda\kappa^Q\).

Shared code: `V4-Models_result/scripts/pq_risk_premium.py`.
