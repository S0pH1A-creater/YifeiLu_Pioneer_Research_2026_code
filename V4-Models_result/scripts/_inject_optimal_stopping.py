...
''',
    ...
    "modified_gbm_v3": {
        "folder": ROOT / "modified gbm v3 notebook",
        "glob": "20*_modified_gbm_v3.ipynb",
        "label": "Modified GBM v3",
        "path_builder": '''
def _rn_paths_for_contract(row, n_paths: int, seed: int):
...
''',
    ...
    "modified_gbm_v2": {
        "folder": ROOT / "modified gbm v2 notebook",
        "glob": "20*_modified_gbm_v2.ipynb",
        "label": "Modified GBM v2",
        "path_builder": '''
def _rn_paths_for_contract(row, n_paths: int, seed: int):
    """Risk-neutral paths to expiry using §5 simulator (mean-corrected E[e^{r_t}]=e^{r Δt})."""
    ticker = str(getattr(row, "underlying", "SPY")).upper()
...
''',
    ...
    "modified_gbm_meanfix": {
        "folder": ROOT / "modified gbm meanfix notebook",
        "glob": "20*_modified_gbm_meanfix.ipynb",
        "label": "Modified GBM meanfix",
        "path_builder": '''
def _rn_paths_for_contract(row, n_paths: int, seed: int):
    """Risk-neutral paths to expiry using §5 simulator (mean-corrected E[e^{r_t}]=e^{r Δt})."""
    ticker = str(getattr(row, "underlying", "SPY")).upper()
...
''',
    ...
    "modified_gbm": {
        "folder": ROOT / "modified gbm notebook",
        "glob": "20*_modified_gbm.ipynb",
        "label": "Modified GBM",
        "path_builder": '''
def _rn_paths_for_contract(row, n_paths: int, seed: int):
    """Risk-neutral paths to expiry using §5 simulator (mean-corrected E[e^{r_t}]=e^{r Δt})."""
...
def _vol_phrase(label: str) -> str:
    ...
    if label == "GARCH":
        return "Duan LRNVR Q-dynamics from §4 for that ticker"
    if label == "Modified GBM":
        return "transition probabilities and up/down magnitudes from §4 for that ticker"
    if label == "Modified GBM v2":
        return "sign coins, calm/wild coins, and lognormal sizes from §4 for that ticker; Q is size-only (\(R\\leftarrow\\lambda R\))"
    if label == "Modified GBM v3":
        return "eight UUU…DDD sign coins and up/down magnitudes from §4 for that ticker"
...
Paths for pricing are **risk-neutral** (drift $\\mu \\rightarrow r$ from the option panel; {_vol_phrase(label)}). §5's expected-vs-history plot is visualization only — it is not the input to LSM.
