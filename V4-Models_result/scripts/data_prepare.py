...
try:
    ...
    DATA_DIR = Path(__file__).resolve().parents[2] / "research" / "data"
    EQUITY_DIR = DATA_DIR / "equity"
    PRICES_PATH = EQUITY_DIR / "prices_clean.csv"
    PRIMARY_TICKER = "SPY"
    SECONDARY_TICKERS = ["AAPL", "MSFT"]
    EXTRA_STUDY_TICKERS = ["AMZN"]
    TICKERS = [PRIMARY_TICKER, *SECONDARY_TICKERS, *EXTRA_STUDY_TICKERS, "JPM", "XOM"]
...
FIGURE_REGIME_LABELS = {
    "crisis": "Crisis (Aug 2008 – Jul 2009)",
    "normal": "Normal (2014)",
    "late": "Late-cycle (Oct 2018 – Sep 2019)",
    "covid": "COVID (Sep 2019 – Aug 2020)",
}
REGIME_COLORS = {
    "crisis": "#c44e52",
    "normal": "#4c72b0",
...
def plot_return_distributions(regime_long: pd.DataFrame) -> Path:
    ...
    for ax, name in zip(axes, REGIME_ORDER):
        ...
        ax.set_title(FIGURE_REGIME_LABELS[name])
...
def plot_all_tickers_normalized(prices: pd.DataFrame) -> Path:
    """Normalized price paths for SPY / AAPL / MSFT / AMZN (+ auxiliary JPM / XOM)."""
    fig, ax = plt.subplots(figsize=(12, 5))
...
def write_research_figures(prices: pd.DataFrame | None = None) -> None:
    """Write 01–05 into research/figures, including AMZN and evaluation-window bands."""
