#!/usr/bin/env python3
"""Switch 1d/7d §6 from forced window-close expiry to listed/natural expiry."""
...
NOTEBOOKS = [
    ...
    ROOT / "heston notebook" / "7d_1min_heston.ipynb",
    ROOT / "heston notebook" / "1d_1min_heston.ipynb",
...
OLD_STEPS = """    n_steps = int(getattr(row, "n_steps", 0)) or n_steps_to_expiry(
        prices.index, row.trading_date, PERIOD_END
    )
    if n_steps < 2:
        raise ValueError("n_steps must be >= 2")"""

NEW_STEPS = """    dte = int(row.dte)
    if dte < 2:
...
HELPER = '''def _with_option_days(fn, *args, **kwargs):
...
Same sampling as the 2-year notebooks: 24 listed American calls, seed 42, OTM/ATM/ITM × DTE 7–60. **Expiration is the listed expiry** (not the window close). Quote times are unique RTH 1-minute stamps on those quote days.
"""

MD2_1D = """## 2. Strike prices in this period

Same sampling as the 2-year notebooks: 24 listed American calls, seed 42, OTM/ATM/ITM × DTE 7–60. **Expiration is the listed expiry** (not forced to this session). 2023-03-15 is an option expiry, so it is a valid 1-day window; a non-expiry day such as 2023-03-13 is not. Quote times are unique RTH 1-minute stamps.
"""
...
def patch(path: Path) -> list[str]:
    ...
    for cell in nb["cells"]:
        ...
        if "sample_forced_expiry_minute_calls" in new and "for ticker in TICKERS:" in new and "_section2" in new:
            new = CODE2
        ...
        if OLD_STEPS in new:
            ...
            hits.append("n_steps=dte")
        if "return simulate_gbm_rolling(" in new and "def _rn_paths_for_contract" in new:
            new = new.replace("return simulate_gbm_rolling(", "return _with_option_days(simulate_gbm_rolling, ")
            hits.append("wrap gbm")
        if "return simulate_merton_rolling(" in new and "def _rn_paths_for_contract" in new:
            new = new.replace("return simulate_merton_rolling(", "return _with_option_days(simulate_merton_rolling, ")
            hits.append("wrap merton")
        if "return simulate_heston_rolling(" in new and "def _rn_paths_for_contract" in new:
            new = new.replace(
                "return simulate_heston_rolling(",
                "return _with_option_days(simulate_heston_rolling, ",
            )
            hits.append("wrap heston")
        if "return simulate_heston_merton_rolling(" in new and "def _rn_paths_for_contract" in new:
            new = new.replace(
                "return simulate_heston_merton_rolling(",
                "return _with_option_days(simulate_heston_merton_rolling, ",
            )
            hits.append("wrap heston")
        if "return simulate_garch_merton_rolling(" in new and "def _rn_paths_for_contract" in new:
            new = new.replace(
        ...
        if OLD_CONTRACTS in new:
            ...
            hits.append("listed sample")
        if "(forced expiry {PERIOD_END}; unique 1-minute times; RMSE = LSM vs contract price)." in new:
            new = new.replace(
                '        f"(forced expiry {PERIOD_END}; unique 1-minute times; RMSE = LSM vs contract price)."',
                '        f"(listed expiry; unique 1-minute quote times; RMSE = LSM vs market)."',
            )
            hits.append("caption")
        if "            dt = 1.0 / N_DAYS\n            stopping_results = {}" in new and "def _rn_paths_for_contract" in new:
            new = new.replace(
                "            dt = 1.0 / N_DAYS\n            stopping_results = {}",
                "            dt = 1.0 / 252\n            stopping_results = {}",
            )
        ...
        if "OPT_DIR = DATA / \"options\" / \"processed\" / \"short_interval\"\n\n_frames = []" in new:
            new = new.replace(
                "OPT_DIR = DATA / \"options\" / \"processed\" / \"short_interval\"\n\n_frames = []",
                "OPT_DIR = DATA / \"options\" / \"processed\" / \"short_interval\"\n\n" + HELPER + "_frames = []",
            )
            hits.append("restore N=252 wrap")
        ...
        new = new.replace(
            ...
            "§6 uses the **same listed-expiry sampling/LSM/RMSE logic as the 2-year files** (natural expiration, DTE 7–60), with unique minute-accurate quote times.",
        )
        new = new.replace(
            ...
            "LSM on 24 listed contracts (seed 42, natural expiry, DTE 7–60), unique minute-accurate quote times; results in `stopping_results`.",
        )
        if "natural expiration" in new or "natural expiry" in new:
            if "intro" not in hits:
                hits.append("intro")
