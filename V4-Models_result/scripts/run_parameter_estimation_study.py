...
Writes under V4-Models_result/results/parameters/:
...
SHORT = REPO / "Results_In_Short" / "V3"
...
STUDIES = [
    ...
    ("Modified GBM", "modified gbm notebook", "20*_modified_gbm.ipynb", "modified_gbm"),
...
MODEL_PANELS = {
    ...
    "modified_gbm": [
        ("p_uu", "P̂(U|U)", "Up persistence"),
        ("p_dd", "P̂(D|D)", "Down persistence"),
        ("mu_u", "μ̂_U", "Up magnitude mean"),
        ("mu_d", "μ̂_D", "Down magnitude mean"),
        ("sig_u", "σ̂_U", "Up magnitude SD"),
        ("sig_d", "σ̂_D", "Down magnitude SD"),
    ],
    "merton": [
        ("mu", "μ̂ (annual)", "Estimated drift"),
...
def _regime_from_name(path: Path) -> str:
    stem = path.stem
    for suffix in ("_heston_merton", "_garch_merton", "_modified_gbm_v3", "_modified_gbm_v2", "_modified_gbm", "_heston", "_garch", "_merton", "_gbm"):
...
def _model_key_from_stem(stem: str) -> str:
    for key in ("heston_merton", "garch_merton", "modified_gbm_v3", "modified_gbm_v2", "modified_gbm", "heston", "garch", "merton", "gbm"):
        ...
        metas.append(
            {
                ...
                "panel_rel": f"../../V4-Models_result/results/parameters/{mode}/{stem}/panel.png",
...
**Source:** `V4-Models_result/results/parameters/{rolling_mode}/`  
Lookback fixed at **3 years**. Parameters estimated **separately** for AAPL / MSFT / SPY (same graph).
...
> **model=`{m['model']}` · regime=`{m['regime']}` · rolling=`{rolling_mode}` · lookback=3 years**
