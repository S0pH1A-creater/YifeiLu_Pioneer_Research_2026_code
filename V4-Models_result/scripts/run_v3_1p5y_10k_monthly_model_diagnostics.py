...
REPORT_TITLE = "V3 model diagnostic verification"
...
def _residuals(model: str, r: np.ndarray):
    ...
    if model == "GBM":
        ...
        if not np.isfinite(sig) or sig <= 0:
            return None
        return (x - mu) / sig
    if model in ("Modified GBM", "Modified GBM meanfix", "MD-GBM"):
        up = x > 0
        mag = np.abs(x)
        ...
        return z
    if model == "Modified GBM v2":
        mag = np.maximum(np.abs(x), 1e-12)
        med = float(np.median(mag))
        up = x > 0
        high = mag > med
        z = np.full_like(x, np.nan, dtype=float)
        for mask in (up & ~high, up & high, ~up & ~high, ~up & high):
            if int(mask.sum()) < 3:
                continue
            logs = np.log(mag[mask])
            mu = float(logs.mean())
            sig = float(logs.std(ddof=1))
            if not np.isfinite(sig) or sig <= 0:
                continue
            z[mask] = (logs - mu) / sig
        z = z[np.isfinite(z)]
        return z if z.size >= 8 else None
    if model == "GARCH":
...
def _cover(pdf: PdfPages) -> None:
    ...
    fig.text(0.08, 0.955, REPORT_TITLE, fontsize=17, weight="bold", color=NAVY, va="top")
    fig.text(
        0.08,
        0.912,
    ...
    y = section(
        ...
        [
            ...
            "GBM: (R_t − μ̂) / σ̂.  "
            + (
                "MD-GBM (Markov Directional Geometric Brownian Motion): split-normal magnitude residual, up vs down.  "
                if "MD-GBM" in emp.TABLE_MODELS
                else "Modified GBM meanfix: split-normal magnitude residual, up vs down.  "
                if "Modified GBM meanfix" in emp.TABLE_MODELS
                else "Modified GBM v2: log-size residual in sign × calm/wild buckets.  "
                if "Modified GBM v2" in emp.TABLE_MODELS
                else "Modified GBM: split-normal magnitude residual, up vs down.  "
                if "Modified GBM" in emp.TABLE_MODELS
                else ""
            )
            + "GARCH: GARCH(1,1) z_t.  Merton: jump-adjusted diffusion σ.  GARCH–Merton / Heston–Merton: residuals after the 3σ jump filter.  Heston: filtered variance path from returns.",
...
Specification tests on realized returns in the four windows of `{STUDY_NAME}`. This report does **not** price options and does **not** use LSM RMSE or MAE.
