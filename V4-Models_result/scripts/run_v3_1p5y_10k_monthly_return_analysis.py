...
One PDF + notebook: four periods × (statistics table + four return charts).
...
REPORT_TITLE = "V3 return analysis"
STUDY_NAME = "V3 1.5-year monthly empirical study 10000 paths"
NAVY = emp.NAVY
MUTED = emp.MUTED
ALT_BG = emp.ALT_BG
...
def _names_phrase() -> str:
    names = list(emp.TICKERS)
    if not names:
        return ""
    if len(names) == 1:
        return names[0]
...
def _table_note(period: dict) -> str:
    ...
    return (
        ...
        f"R_t = ln(S_t / S_{{t-1}}), for {_names_phrase()}. The sample period is {start} to {end}, "
...
def _figure_note(period: dict) -> str:
    ...
    n = period["n"]
    return (
        f"Notes: This figure plots the annualized continuously compounded returns 252 × R_t for "
        f"{_names_phrase()}. Date is on the x-axis and the annualized return, in percent, is on the "
        f"y-axis. Each panel is one name. The shaded band is the evaluation window {start} to {end} "
        f"({n} trading days); six months of returns are shown on each side so that band is visible. "
        f"Large spikes are one-day moves scaled by 252, not cumulative performance."
    )
...
def _cover(pdf: PdfPages) -> None:
    ...
    fig.text(0.08, 0.955, REPORT_TITLE, fontsize=18, weight="bold", color=NAVY, va="top")
    fig.text(
        0.08,
        0.912,
        f"{_names_phrase()}  ·  four study windows  ·  {emp.LOOKBACK_PHRASE} monthly study",
        fontsize=9.2,
        color=MUTED,
        va="top",
    )
    ...
    y = section(
        ...
        [
            ...
            f"One table and one four-panel time-series figure per window, always {_names_phrase()}, always the same five rows and the same return definition.",
...
def _draw_return_figure(period: dict, idx: int, *, for_pdf: bool = True):
    ...
    for ax, ticker in zip(axes, tickers):
        log_r = np.log(prices[ticker]).diff()
        series = log_r.loc[left:right].dropna()
        dates = series.index
        y = (252.0 * 100.0 * series).to_numpy(dtype=float)
        ax.axvspan(start, end, color=NAVY, alpha=0.12, zorder=0, label=period["title"] if ticker == tickers[0] else None)
        ax.plot(dates, y, color=TICKER_COLORS.get(ticker, NAVY), lw=0.85, solid_capstyle="round", zorder=2)
        ax.axhline(0.0, color="#B0B7C3", lw=0.7, zorder=1)
        ...
        ax.set_xlim(dates.min() if len(dates) else left, dates.max() if len(dates) else right)
    axes[0].legend(loc="upper right", frameon=False, fontsize=8.0)
    axes[-1].set_xlabel("Date", fontsize=8.5, color="#333333")
    axes[-1].xaxis.set_major_locator(mdates.MonthLocator(interval=2))
    axes[-1].xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
    fig.autofmt_xdate(rotation=0, ha="center")
    if for_pdf:
        fig.tight_layout(rect=(0.04, 0.11, 0.98, 0.94))
        _wrapped_note(fig, _figure_note(period), 0.09, width=124, fontsize=7.8)
    else:
        fig.tight_layout()
    return fig
...
def build_notebook(payload: dict, path: Path | None = None) -> Path:
    ...
    cells = [
        md(
            f"""# {REPORT_TITLE} — {emp.LOOKBACK_PHRASE} monthly study

**This notebook is the computational source of truth.** The matching PDF is written from the same payload.

Realized equity returns in the four windows of `{STUDY_NAME}`. This report does not price options and does not resimulate LSM paths.
