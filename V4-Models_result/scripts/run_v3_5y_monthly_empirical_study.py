...
MODEL_FOLDERS = {
    ...
    "Modified GBM meanfix": ("modified gbm meanfix notebook", "20*_modified_gbm_meanfix.ipynb"),
    "MD-GBM": ("modified gbm meanfix notebook", "20*_modified_gbm_meanfix.ipynb"),
    "Modified GBM v2": ("modified gbm v2 notebook", "20*_modified_gbm_v2.ipynb"),
    "Modified GBM v3": ("modified gbm v3 notebook", "20*_modified_gbm_v3.ipynb"),
    "Modified GBM AI": ("modified gbm ai notebook", "20*_modified_gbm_ai.ipynb"),
...
def _figure_pages(pdf: PdfPages, payload: dict) -> None:
    ...
    # bias figure (Bias%)
    fig, axes = plt.subplots(2, 2, figsize=(11, 8.5))
    fig.suptitle(f"Page 12  ·  Figure 4  ·  Bias%  ·  mean of {_companies_phrase()}", fontsize=12.5, color=NAVY, y=0.98, weight="bold")
    for ax, regime in zip(axes.ravel(), REGIME_ORDER):
        vals = [float(np.mean([cells[f"{t}|{regime}|{m}"]["bias_pct"] for t in TICKERS])) for m in TABLE_MODELS]
...
def _analysis_text(payload: dict) -> list[str]:
    ...
    nojump = [m for m in ("GBM", "Modified GBM", "Modified GBM meanfix", "MD-GBM", "Modified GBM v2", "Modified GBM v3", "Modified GBM AI", "GARCH", "Heston") if m in TABLE_MODELS]
...
def _conclusion_page(pdf: PdfPages, payload: dict) -> None:
    ...
    fig.text(
        ...
        f"Engine: V4-Models_result/scripts/{ENGINE_SCRIPT}  ·  payload.json is the shared number store.",
        fontsize=7.6,
        color="#666666",
        va="bottom",
    )
