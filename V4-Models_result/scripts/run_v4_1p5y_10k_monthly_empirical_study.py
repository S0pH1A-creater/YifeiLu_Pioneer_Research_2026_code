#!/usr/bin/env python3
"""V4 1.5-year monthly empirical study, 10,000 LSM paths.

Same seven models and grouping as V3. Same Monday ATM sample, 18-month
lookback, monthly rolling, four companies, four 1-year regimes.

GARCH cells are copied from the finished Duan LRNVR cache and are not re-run.
"""
...
def apply_v4_10k_config() -> None:
    study.configure_lookback(
        ...
        short_name="V4/V4 1.5-year monthly empirical study 10000 paths",
...
def patch_v3_wrap() -> None:
    """Point V3 companion scripts at this V4 10k cache and Results_In_Short folder."""
...
def layout_short_outputs() -> None:
    """PDFs at the folder root, notebooks in Notebooks/, matching V3 monthly."""
    apply_v4_10k_config()
    short = study.SHORT
    ...
    for stale in (
        "V4_1p5y_monthly_empirical_study_pan_q.pdf",
        "V4_1p5y_monthly_empirical_study_pan_q.ipynb",
    ):
        path = short / stale
        if path.exists():
            path.unlink()
        ...
        fig = _page(
            ...
            [
                ...
                (
                    "What it is not",
                    [
                        "Not Modified GBM v2 (no lognormal sizes, no calm/wild chain, no size-only Q).",
                        "Not Heston, not Merton, and not GARCH.",
                    ],
                ),
            ],
            ...
            [
                ...
                (
                    ...
                    [
                        ...
                        "Q-measure (decision / moneyness): after drawing R, add r_f Δt − log E[e^R]. Then Longstaff–Schwartz on the same Monday ATM listed-call sample as every other model.",
                    ],
                ),
                (
                ...
                (
                    "Where results are",
                    [
                        "Seven-model ranking: V4_1p5y_monthly_empirical_study.pdf. Return-based group: V4_1p5y_monthly_empirical_study_return_based.pdf.",
                        "Code: V4-Models_result/modified gbm meanfix notebook/20*_modified_gbm_meanfix.ipynb.",
                    ],
                ),
            ],
...
def swap_modified_gbm_meanfix_cache() -> None:
    ...
    if not src.exists():
        raise FileNotFoundError(f"Need the finished Modified GBM meanfix cache at {src}")
    n_copy = 0
    ...
    for stale in (
        dst / "payload.json",
        dst / "payload_return_based.json",
        dst / "payload_option_implied.json",
        dst / "moneyness_payload.json",
        dst / "stock" / "payload.json",
        dst / "parameter_estimation" / "payload.json",
        dst / "model_diagnostics" / "payload.json",
    ):
        if stale.exists():
            stale.unlink()
...
def copy_modified_gbm_spec() -> None:
    ...
    nb_dir.mkdir(parents=True, exist_ok=True)
    for stale in (
        study.SHORT / "V4_modified_gbm_model.pdf",
        study.SHORT / "V4_modified_gbm_model.ipynb",
        study.SHORT / "V4_modified_gbm_v2_model.pdf",
        study.SHORT / "V4_modified_gbm_v2_model.ipynb",
        study.SHORT / "V4_modified_gbm_meanfix_model.pdf",
        study.SHORT / "V4_modified_gbm_meanfix_model.ipynb",
        nb_dir / "V4_modified_gbm_model.ipynb",
        nb_dir / "V4_modified_gbm_model.pdf",
        nb_dir / "V4_modified_gbm_v2_model.ipynb",
        nb_dir / "V4_modified_gbm_v2_model.pdf",
        nb_dir / "V4_modified_gbm_meanfix_model.ipynb",
        nb_dir / "V4_modified_gbm_meanfix_model.pdf",
    ):
        if stale.exists():
            stale.unlink()
            print(f"removed {stale.name}", flush=True)
    write_modified_gbm_v2_pdf(study.SHORT / "V4_MD-GBM_model.pdf")
    write_modified_gbm_notebook(nb_dir / "V4_MD-GBM_model.ipynb")
...
def merge_garch_cache() -> None:
    ...
    if not src.exists():
        raise FileNotFoundError(
            f"Need the finished GARCH 10k cache at {src}"
        )
