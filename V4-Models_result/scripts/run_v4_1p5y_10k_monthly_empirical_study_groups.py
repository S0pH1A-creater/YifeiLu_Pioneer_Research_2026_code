#!/usr/bin/env python3
"""Split the V4 1.5-year 10k LSM study into V3's two calibration-group reports.

The split is how P is estimated, not the P→Q map:

- return-based: MD-GBM, GBM, GARCH, Merton, GARCH–Merton
...
GROUPS = {
    "option_implied": {
        ...
        "pdf_name": "V4_1p5y_monthly_empirical_study_option_implied.pdf",
        "nb_name": "V4_1p5y_monthly_empirical_study_option_implied.ipynb",
    ...
    "return_based": {
        ...
        "group_intro": [
            "Companion to the seven-model 10,000-path V4 study. This report keeps MD-GBM, GBM, GARCH, Merton, and GARCH–Merton, whose P dynamics are estimated from lookback returns. That is the same V3 split: return-based vs option-implied Heston, not a split by the P→Q map. GBM still uses μ→r_f; GARCH uses Duan LRNVR; Merton uses Pan jump-size premium μ_J* from listed calls on top of return-based P jumps; GARCH–Merton uses Duan LRNVR on the GARCH block plus Pan μ_J* on jumps. MD-GBM means Markov Directional Geometric Brownian Motion: original Modified GBM with folded-normal means matched to sample |R|.",
            "Companies are SPY, AAPL, MSFT, and AMZN. No new calibration or LSM.",
...
ORIGINAL_PDF = "V4_1p5y_monthly_empirical_study.pdf"
ORIGINAL_NB = "V4_1p5y_monthly_empirical_study.ipynb"
