...
Used by LSM in the GBM / MD-GBM / Merton / GARCH / GARCH–Merton notebooks.
...
def pct_mae(model, market, *, min_price: float = 1e-8) -> float:
    """Percentage MAE: 100 × mean(|ŷ − y|/y). Companion to percentage RMSE."""
    yhat = np.asarray(model, dtype=float)
    y = np.asarray(market, dtype=float)
    ok = np.isfinite(yhat) & np.isfinite(y) & (np.abs(y) > float(min_price))
    if not np.any(ok):
        return float("nan")
    return float(100.0 * np.mean(np.abs(yhat[ok] - y[ok]) / y[ok]))
