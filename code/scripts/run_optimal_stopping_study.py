...
For each of GBM / MD-GBM / Merton / GARCH / GARCH–Merton × four regimes:
  lookback = 3 years
  rolling  = none, monthly, daily
  underlyings = SPY, AAPL, MSFT

Write per-ticker markdown + figures under code/results/{TICKER}/,
then regenerate the three compare_rolling_*.ipynb notebooks (same layout per ticker).
Equity/options data are read from ../data (shared with the repo).
"""
...
STUDIES = [
    ("GBM", "gbm", "*_gbm.ipynb", "gbm"),
    ("MD-GBM", "md_gbm", "*_modified_gbm_meanfix.ipynb", "modified_gbm_meanfix"),
    ("Merton", "merton", "*_merton.ipynb", "merton"),
    ("GARCH", "garch", "20*_garch.ipynb", "garch"),
    ("GARCH–Merton", "garch_merton", "*_garch_merton.ipynb", "garch_merton"),
]

# Display order in compare notebooks (regime then model)
REGIME_ORDER = ["2008-2009", "2013-2014", "2018-2019", "2019-2020"]
MODEL_ORDER = {"GBM": 0, "MD-GBM": 1, "Merton": 2, "GARCH": 3, "GARCH–Merton": 4}
...
def _regime_from_name(path: Path) -> str:
    stem = path.stem
    for suffix in ("_heston_merton", "_garch_merton", "_modified_gbm_v3", "_modified_gbm_v2", "_modified_gbm_ai", "_modified_gbm_2lag", "_modified_gbm_meanfix", "_modified_gbm", "_heston", "_garch", "_merton", "_gbm"):
...
**Source:** `code/results/{{TICKER}}/` after methodological fixes (see `V3_CHANGES.md`).  
...
## Layout
For each ticker: regimes 2008-2009 → 2019-2020; within each GBM → MD-GBM → Merton → GARCH → GARCH–Merton.  
