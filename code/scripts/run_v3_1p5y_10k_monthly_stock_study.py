...
STOCK_GROUPS = {
    "return_based": {
        ...
        "group_intro": [
            "P-measure stock-path report. Models: MD-GBM, GBM, GARCH, Merton, and GARCH–Merton. Dynamics come from lookback returns. Rankings are within this set.",
        ],
    },
}
...
def write_grouped_outputs(payload: dict) -> list[tuple[Path, Path]]:
    """Write the return-based stock PDF from stored cells. No new paths."""
    written = []
    for group_id in STOCK_GROUPS:
...
def write_outputs(payload):
    cache, _, _ = _paths()
    (cache / "payload.json").write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    return write_grouped_outputs(payload)
