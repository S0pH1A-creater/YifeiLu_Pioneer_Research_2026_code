...
The notebook under results/1p5y_monthly_return_based is the user-facing
source of truth; this module is the computational engine it imports.
The PDF is written from the same payload dict so every number matches.
"""
...
from american_lsm import load_calls, pct_mae, pct_rmse, sample_calls  # noqa: E402
...
MODELS = ("GBM", "MD-GBM", "GARCH", "Merton", "GARCH–Merton")
TABLE_MODELS = ("GBM", "MD-GBM", "GARCH", "Merton", "GARCH–Merton")
...
MODEL_FOLDERS = {
    "GBM": ("gbm", "20*_gbm.ipynb"),
    "MD-GBM": ("md_gbm", "20*_modified_gbm_meanfix.ipynb"),
    "Modified GBM meanfix": ("md_gbm", "20*_modified_gbm_meanfix.ipynb"),
    "GARCH": ("garch", "20*_garch.ipynb"),
    "Merton": ("merton", "20*_merton.ipynb"),
    "GARCH–Merton": ("garch_merton", "20*_garch_merton.ipynb"),
}

CACHE = ROOT / "results" / "empirical_study_5y_monthly"
SHORT = REPO / "results" / "1p5y_monthly_return_based"
...
MODEL_COLORS = {
    "GBM": "#4C72B0",
    "Modified GBM": "#C73E7B",
    "Modified GBM meanfix": "#9B4D7A",
    "MD-GBM": "#9B4D7A",
    "MD-GBM 2-lag": "#6D3B6E",
    "Modified GBM v2": "#8E3B67",
    "Modified GBM v3": "#B85C38",
    "Modified GBM AI": "#6B2D5B",
    "GARCH": "#C44E52",
    "Heston": "#DD8452",
    "Merton": "#55A868",
    "GARCH–Merton": "#8172B3",
...
MODEL_COVER_NAMES = {
    ...
    "Modified GBM meanfix": "Modified GBM meanfix (folded-normal E[|N|])",
    "MD-GBM": "MD-GBM (Markov Directional Geometric Brownian Motion)",
    "MD-GBM 2-lag": "MD-GBM 2-lag (order-2 sign chain)",
    "Modified GBM v2": "Modified GBM v2 (lognormal + calm/wild)",
    "Modified GBM v3": "Modified GBM v3 (UUU…DDD sign history)",
    "Modified GBM AI": "Modified GBM AI (Adam / PyTorch)",
    "GARCH": "GARCH(1,1)",
...
def sample_shared_contracts() -> dict:
    ...
    for nb in sorted((ROOT / "gbm").glob("20*_gbm.ipynb")):
...
def enrich_bias_pct(payload: dict) -> dict:
    """Fill Bias% and MAE% from saved contract CSVs so a frozen run still gets them."""
...
def _run_one(model: str, nb_path: Path, shared: dict, tickers: tuple[str, ...] | list[str] | None = None) -> dict:
    ...
    for ticker in names:
        ...
        print(
            ...
            f"MAE%={mets['mae']:.2f} n={mets['n']} updates={mets['n_updates']} "
...
CACHE_MODEL_TO_DISPLAY = {
    "Modified GBM meanfix": "MD-GBM",
}


def file_stem(model: str) -> str:
    if model in MODEL_FILE_STEM:
        return MODEL_FILE_STEM[model]
    return model.replace("–", "-").replace(" ", "_")


def display_model(model: str) -> str:
    return CACHE_MODEL_TO_DISPLAY.get(model, model)


def _partial_path(model: str, regime: str) -> Path:
    return CACHE / "partial" / f"{regime}_{file_stem(model)}.json"
...
def run_study(only: list[str] | None = None) -> dict:
    ...
    if only:
        ...
        model_alias = {
            ...
            "modified-gbm-meanfix": "Modified GBM meanfix",
            "md-gbm": "MD-GBM",
            "md-gbm-2-lag": "MD-GBM 2-lag",
            "modified-gbm-2lag": "MD-GBM 2-lag",
            "modified-gbm-v2": "Modified GBM v2",
            "modified-gbm-v3": "Modified GBM v3",
            "modified-gbm-ai": "Modified GBM AI",
            "garch": "GARCH",
            "heston": "Heston",
            "merton": "Merton",
            "garch-merton": "GARCH–Merton",
...
def assemble_payload(shared, funnel, completed, failures, elapsed) -> dict:
    ...
    return {
        "meta": {
            ...
            "mae_definition": "100 × mean(|model − market| / market)",
            ...
            (
                ...
                [
                    ...
                    for b in [
                        ...
                        (
                            "Modified GBM meanfix (folded-normal E[|N|] matched to mean |R|) is specified in V4_modified_gbm_meanfix_model.pdf in this folder."
                            if "Modified GBM meanfix" in TABLE_MODELS
                            else None
                        ),
                        (
        ...
        [
            (
                ...
                [
                    ...
                    "Estimation filters, applied in order to the LSM comparison set: (1) calls only, finite S, K, C; (2) no-arbitrage C ≥ max(0, S−K); (3) 7 ≤ DTE ≤ 60; (4) near ATM |S/K − 1| ≤ 10%; (5) premium ≥ 0.05, valid bid–ask with relative spread ≤ 50% when those columns exist, volume ≥ 1 when present.",
                    "P dynamics are estimated from lookback log returns. Merton / GARCH–Merton jump-size premia for Q still use listed calls (Pan). LSM prices the same filtered Monday ATM sample for every model.",
                ],
            ),
            (
                "Model-correctness adjustments (V3)",
                [
                    "GARCH uses Duan LRNVR. Merton uses Pan μ_J* from listed calls. GBM / MD-GBM use an additive shift so E[e^R]=e^{r_f Δt}.",
            ...
            (
                ...
                [
                    ...
                    "Secondary: MAE% = 100 × mean(|C_model − C_mkt|/C_mkt). Bias% = 100 × mean((C_model − C_mkt)/C_mkt). Positive bias = model expensive vs market.",
                ],
            ),
        ],
...
def _filters_page(pdf: PdfPages, payload: dict) -> None:
    ...
    fig.text(
        0.08,
        0.055,
        "GBM / MD-GBM / GARCH estimate from lookback log returns only. Merton / GARCH–Merton add 3σ residual jumps; Q jump-size premia use listed calls.",
        fontsize=8.0,
        color="#222222",
        va="top",
        wrap=True,
...
def _company_tables_page(pdf: PdfPages, payload: dict, ticker: str, page_no: int) -> None:
    ...
    fig.text(
        0.06,
        0.03,
        "RMSE% is 100×√mean(((model−market)/market)²).  MAE% is 100×mean(|model−market|/market).  "
        "Bias% is 100×mean((model−market)/market).  "
...
def _metric_vals_for_names(cells: dict, regime: str, model: str, names: tuple[str, ...], key: str) -> float:
    vals = []
    for t in names:
        rec = cells.get(f"{t}|{regime}|{model}")
        if rec is None:
            continue
        v = rec.get(key)
...
def _regime_window_text(regime: str) -> str:
    """Full date span for a regime, e.g. '2014-01-01 to 2014-12-31'."""
    raw = REGIME_META[regime]["window"]
    return raw.replace("→", "to").replace("  ", " ").strip()


def _draw_main_table_bar_grid(
    fig,
    axes,
    *,
    cells: dict,
    ...
    """Eight panels matching the main tables: SPY × 4 regimes, company mean × 4 regimes."""
    names = _equity_tickers()
    x = np.arange(len(TABLE_MODELS))
    label_fs = 7.4 if len(TABLE_MODELS) >= 5 else 8.2
    value_fs = 7.2 if len(TABLE_MODELS) >= 5 else 8.0
    title_fs = 7.6
    for col, regime in enumerate(REGIME_ORDER):
        window_txt = _regime_window_text(regime)
        for row, (asset_label, tickers) in enumerate(
            (
                ("SPY asset", ("SPY",) if "SPY" in TICKERS else (TICKERS[0],)),
                ("Company asset", names if names else TICKERS),
            )
        ):
            ax = axes[row, col]
            vals = [_metric_vals_for_names(cells, regime, m, tickers, metric_key) for m in TABLE_MODELS]
            finite = [v for v in vals if np.isfinite(v)]
            bars = ax.bar(x, [0.0 if not np.isfinite(v) else v for v in vals], color=[MODEL_COLORS[m] for m in TABLE_MODELS], width=0.72, zorder=3)
            if outline == "absmin" and finite:
                ax.axhline(0.0, color="#333333", lw=0.7)
                best_i = int(np.nanargmin(np.abs(np.asarray(vals, dtype=float))))
            elif finite:
                best_i = int(np.nanargmin(np.asarray(vals, dtype=float)))
            else:
                best_i = 0
            bars[best_i].set_edgecolor("#111111")
            bars[best_i].set_linewidth(1.35)
            if finite and outline != "absmin":
                ymax = max(finite)
                ymin = 0.0
                for bar, v in zip(bars, vals):
                    if not np.isfinite(v):
                        continue
                    ax.text(bar.get_x() + bar.get_width() / 2, v + 0.02 * max(ymax, 1e-9), f"{v:.1f}", ha="center", va="bottom", fontsize=value_fs)
                ax.set_ylim(ymin, ymax * 1.32 if ymax > 0 else 1)
            elif finite and outline == "absmin":
                for bar, v in zip(bars, vals):
                    if not np.isfinite(v):
                        continue
                    va = "bottom" if v >= 0 else "top"
                    ax.text(bar.get_x() + bar.get_width() / 2, v, f"{v:.1f}", ha="center", va=va, fontsize=value_fs)
            ax.set_title(
                f"{asset_label}  ·  Regime {col + 1}\n{window_txt}",
                loc="left",
                fontsize=title_fs,
                color=NAVY,
                weight="bold",
            ...
            ax.set_xticks(x)
            ax.set_xticklabels(list(TABLE_MODELS), fontsize=label_fs, rotation=22, ha="right")
            if col == 0:
...
def _figure_pages(pdf: PdfPages, payload: dict) -> None:
    cells = payload["cells"]
    names = _equity_tickers()
    names_txt = ", ".join(names) if names else _tickers_joined()

    # Eight panels = eight main tables (SPY asset + Company asset × four regimes)
    fig, axes = plt.subplots(2, 4, figsize=(11, 8.5))
    fig.suptitle(
        "Page 9  ·  Figure 1  ·  RMSE% for the eight main tables  ·  SPY asset and Company asset × four regimes",
        fontsize=11.0,
        color=NAVY,
        y=0.985,
        weight="bold",
    )
    _draw_main_table_bar_grid(fig, axes, cells=cells, metric_key="rmse_pct", ylabel="RMSE%", outline="min")
    fig.text(
        0.04,
        0.02,
        f"Top row = SPY asset, Regime 1–4.  Bottom row = Company asset, Regime 1–4 (arithmetic mean of {names_txt}).  "
        "Outlined bar = lowest RMSE% in that panel.",
        fontsize=7.4,
        color="#555555",
    )
    ...
    # bias figure (Bias%) — same eight main-table panels as Figure 1
    fig, axes = plt.subplots(2, 4, figsize=(11, 8.5))
    fig.suptitle(
        "Page 12  ·  Figure 4  ·  Bias% for the eight main tables  ·  SPY asset and Company asset × four regimes",
        fontsize=11.0,
        color=NAVY,
        y=0.985,
        weight="bold",
    )
    _draw_main_table_bar_grid(fig, axes, cells=cells, metric_key="bias_pct", ylabel="Bias%", outline="absmin")
    fig.text(
        0.04,
        0.02,
        f"Top row = SPY asset, Regime 1–4.  Bottom row = Company asset, Regime 1–4 (arithmetic mean of {names_txt}).  "
...
def _analysis_text(payload: dict) -> list[str]:
    ...
    nojump = [m for m in ("GBM", "Modified GBM", "Modified GBM meanfix", "MD-GBM", "MD-GBM 2-lag", "Modified GBM v2", "Modified GBM v3", "Modified GBM AI", "GARCH", "Heston") if m in TABLE_MODELS]
...
def _conclusion_page(pdf: PdfPages, payload: dict) -> None:
    ...
    fig.text(
        ...
        f"Engine: code/scripts/{ENGINE_SCRIPT}  ·  payload.json is the shared number store.",
        fontsize=7.6,
        color="#666666",
        va="bottom",
    )
...
| Code | `code/` model notebooks + `option_filters.py` |

{"**MD-GBM** means Markov Directional Geometric Brownian Motion. Specification: `V4_MD-GBM_model.pdf` in this folder." if "MD-GBM" in TABLE_MODELS else ""}
...
- GARCH: Duan LRNVR. Merton: Pan μ_J*. GBM / MD-GBM: additive Q shift.
...
"""
    ...
    def draw_fig1():
        fig, axes = plt.subplots(2, 4, figsize=(11, 7.8))
        fig.suptitle(
            "Figure 1 · RMSE% for the eight main tables · SPY asset and Company asset × four regimes",
            color=NAVY,
            fontsize=11.0,
            weight="bold",
        )
        _draw_main_table_bar_grid(
            fig,
            axes,
            cells=payload["cells"],
            metric_key="rmse_pct",
    ...
    names_fig = ", ".join(_equity_tickers()) if _equity_tickers() else _tickers_joined()
    cells.append(md(f"### Figure 1 — RMSE% for the eight main tables (SPY asset and Company asset × Regime 1–4; Company asset = mean of {names_fig})"))
